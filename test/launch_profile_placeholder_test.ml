...
let test_profile_placeholder_requires_selected_profile () =
  let session =
    make_session
      ~title:"Launch placeholder regression"
      ~project_key:"sessy"
      "profile-missing-1"
  in
  let template =
...
let test_session_data_with_profile_literal_is_not_reinterpreted () =
  ...
  | Ok launch_cmd ->
      check (list string)
        "session data is preserved verbatim"
        [ "resume"; literal_title ]
        (launch_cmd.argv |> snd)
  | Error _ -> fail "session data should not be reinterpreted as template syntax"
    ...
    [
      ( "launch",
        [
          ...
          test_case
            "session data containing {{profile}} stays literal"
            `Quick
